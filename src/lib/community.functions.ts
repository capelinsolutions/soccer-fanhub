...
export const listFans = createServerFn({ method: "GET" })
  .inputValidator((input?: { teamId?: number | null; limit?: number }) => ({
...
export const getTeamFanCounts = createServerFn({ method: "GET" }).handler(
  async () => {
    const sb = publicClient();
    const { data } = await sb
...
export const getCountryFanCounts = createServerFn({ method: "GET" }).handler(
  async () => {
    const sb = publicClient();
    const { data } = await sb
      .from("profiles")
