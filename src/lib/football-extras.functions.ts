...
export const getWcTeams = createServerFn({ method: "GET" }).handler(
  async (): Promise<WcTeam[]> => {
    try {
...
export const getTeamDetail = createServerFn({ method: "GET" })
  .inputValidator((input: { teamId: number | string }) => ({
    teamId: Number(input.teamId),
  }))
  .handler(async ({ data }) => {
    const { getTeamById, getTeamPlayers, getAllMatches, getWcTeams } = await import(
      "./football-cache.server"
    );
    ...
    const teamMatches = (matchesRes.data ?? [])
      .filter(
        (m: any) =>
          m.home_team_id === data.teamId || m.away_team_id === data.teamId,
      )
      .map((m: any) => ({
        ...m,
...
export const getStandings = createServerFn({ method: "GET" }).handler(
  async () => {
    try {
      ...
      const teamById = new Map<number, { name: string | null; crest: string | null; tla: string | null }>();
      for (const t of (teamsRes.data ?? []) as any[]) {
